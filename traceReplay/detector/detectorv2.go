...
var varStates map[uint32]int = make(map[uint32]int)
var filteredEvents uint64

func unsoundEventFilter(in, out chan *util.Item) {
	for it := range in {
		state, ok := varStates[it.Ops[0].Ch]

		if !ok {
			varStates[it.Ops[0].Ch] = int(it.Thread)
			state = int(it.Thread)
		}

		if state == -1 {
			out <- it
		} else if uint32(state) != it.Thread {
			varStates[it.Ops[0].Ch] = -1
			out <- it
		} else {
			filteredEvents++
		}
...
func RunAPISingleInc(detector string, withPostProcess bool, trace string, n, a uint64, filter uint64) {
	traceReplay.EvListener = []traceReplay.EventListener{algos.GetDetector(detector)}

	items := make(chan *util.Item, 100000)
	go traceParser.ParseJTracevInc(trace, n, a, items)

	if filter == 1 {
		filteredItems := make(chan *util.Item, 100000)
		go unsoundEventFilter(items, filteredItems)
		items = filteredItems
	}
	...
	if filter == 1 { //report filter statistics
		fmt.Println("Filtered Events:", filteredEvents)
		sharedVars := 0
		for _, v := range varStates {
			if v == -1 {
				sharedVars++
			}
		}
		fmt.Printf("All Vars:%v\nShared Vars:%v\n", len(varStates), sharedVars)
	}
}

func RunAPISingleIncDouble(detector string, withPostProcess bool, trace string, n, a uint64, repeat int) {
	traceReplay.EvListener = []traceReplay.EventListener{algos.GetDetector(detector)}
