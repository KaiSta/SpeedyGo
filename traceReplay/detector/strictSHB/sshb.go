	...
	if p.Write {
		...
		for i, w := range varstate.writes {
			...
			if k > curr {
				...
				b := report.ReportRaceDRY(l1, l2, false, 0)
				if b {
					r.acc2.clock2 = t1.vc.clone().ssync(t1.lw).trim()
					varstate.races = append(varstate.races, r)
				}
				...
				for _, n := range w.next {
					if n.ev.Ops[0].Kind&util.READ > 0 && n.ev.Ops[0].Ch == w.ev.Ops[0].Ch {
						n.concW = append(n.concW, varstate.writes[i])
					}
				}
			}
		}
...
func checkRaceValidity(r race) bool {
	valid, _, _ := searchWPath(r.acc1, r.acc2, make(map[*node]struct{}), 0, []*node{})
...
func search(current, finish *node, visited map[*node]struct{}, level int) (bool, int) {
	...
	for i := range current.next {
		if current.next[i] == finish { //if level != 0 && current.next[i] == finish {
			return true, level
		}
	}
...
func countAlternativeWrites() {
	...
	fmt.Printf("Categorising:\n =0:%v\n=1:%v\n>=2:%v\n>=5:%v\n>=10:%v\n", zeroWrites, oneWrite, twoWrites, fiveWrites, tenWrites)
}
...
func (l *ListenerPostProcessFIX) Put(p *util.SyncPair) {
	...
	for _, v := range variables {
		...
		for i, w := range v.writes {
			for _, n := range w.next {
				if n.ev.Ops[0].Kind&util.READ > 0 && n.ev.Ops[0].Ch == w.ev.Ops[0].Ch {
					n.concW = append(n.concW, v.writes[i])
				}
			}
		}
	}
	...
	for _, v := range variables {
		for _, r := range v.races {
			//fmt.Println("RACE:", r.acc1.ev.ShortString(), r.acc2.ev.ShortString())
			if !r.lsEmpty {
				continue
			}
			l1 := report.Location{File: r.acc1.ev.Ops[0].SourceRef, Line: r.acc1.ev.Ops[0].Line, W: r.acc1.ev.Ops[0].Kind&util.WRITE > 0}
			...
			ok, p, _ := searchWPath(r.acc1, r.acc2, make(map[*node]struct{}), 0, []*node{})

			if !ok {
				color.HiGreen("GUARANTEED RACE\n")
				guaranteed++
			} else {
				color.HiYellow("MAYBE RACE\n")
				if len(p) == 1 {
					color.HiYellow("WRD Race")
					wrd++
				} else {
					maybe++
				}
			...
			lastW := make(map[uint32]*node)
			unsyncedWRD := 0
			//involvedWrites := 0
			unilocWrites := make(map[string]uint32)
			unilocWRDPairs := make(map[string]uint32)
			uniqueWritesInWRDs := make(map[string]struct{})
			uniqueReadsInWRDs := make(map[string]struct{})

			for _, x := range p {
				if x.ev.Ops[0].Kind&util.WRITE > 0 {
					id := x.ev.Ops[0].Ch
					lastW[id] = x
				} else if x.ev.Ops[0].Kind&util.READ > 0 {
					id := x.ev.Ops[0].Ch
					y, ok := lastW[id]
					if ok {
						if y.clock.get(x.ev.Thread) < x.clock.get(x.ev.Thread) { //concurrent
							unsyncedWRD++

							s := fmt.Sprintf("%v:%v,%v:%v", y.ev.Ops[0].SourceRef, y.ev.Ops[0].Line, x.ev.Ops[0].SourceRef, x.ev.Ops[0].Line)
							unilocWRDPairs[s] = y.ev.Ops[0].Ch

							ws := fmt.Sprintf("%v:%v", y.ev.Ops[0].SourceRef, y.ev.Ops[0].Line)
							rs := fmt.Sprintf("%v:%v", x.ev.Ops[0].SourceRef, x.ev.Ops[0].Line)
							uniqueWritesInWRDs[ws] = struct{}{}
							uniqueReadsInWRDs[rs] = struct{}{}

							// fmt.Printf("UnsynchronizedWRDs:\n%v-%v\n", y.ev.ShortString(), x.ev.ShortString())
							// fmt.Printf("Involved Writes:\n")
							unilocConcurWrites := make([]*node, 0)

							for _, z := range x.concW {
								s := fmt.Sprintf("%v:%v", z.ev.Ops[0].SourceRef, z.ev.Ops[0].Line)
								unilocWrites[s] = z.ev.Ops[0].Ch

								newWrites := make([]*node, 0)
								for _, w := range unilocConcurWrites {
									k := w.clock.get(w.ev.Thread)
									curr := z.clock.get(w.ev.Thread)
									if k >= curr {
										newWrites = append(newWrites, w)
									}
								}
								newWrites = append(newWrites, z)
								unilocConcurWrites = newWrites
							}
							if len(unilocConcurWrites) > 1 {
								color.HiRed("more than one write candidate for wrd! %v", len(unilocConcurWrites))

								for _, w := range unilocConcurWrites {
									fmt.Println(w.ev.Thread, "$", w.ev.Ops[0].SourceRef, ":", w.ev.Ops[0].Line, "-", w.ev.Ops[0].Ch)
								}
							}

							// else if len(unilocConcurWrites) == 1 {
							// 	fmt.Println(unilocConcurWrites[0].ev.Ops[0].SourceRef, unilocConcurWrites[0].ev.Ops[0].Line)
							// }
			...
			if len(p) < 7 {
				for _, x := range p {
					fmt.Printf("%v->", x.ev.ShortString())
				}
				fmt.Printf("\n")
			}
			fmt.Println("unsynchronized WRDS:", len(unilocWRDPairs))
			for k, v := range unilocWRDPairs {
				fmt.Println(k, v)
			}
			fmt.Println("------")
			fmt.Println("involvedWrites:", len(unilocWrites))
			for k, v := range unilocWrites {
				fmt.Println(k, v)
			}
			fmt.Println("------")
			fmt.Println("writes in wrd", len(uniqueWritesInWRDs))
			fmt.Println("reads in wrd", len(uniqueReadsInWRDs))
			fmt.Println("----------------------")
			steps++
			// if steps%stepBarrier == 0 {
			fmt.Printf("\r%v/%v", steps, complete)
	...
	fmt.Printf("GUARANTEED:%v\nMAYBE:%v\nWRD:%v\n", guaranteed, maybe, wrd)

	report.ReportFinish()
}

func searchWPath(current, finish *node, visited map[*node]struct{}, level int, path []*node) (bool, []*node, int) {

	visited[current] = struct{}{}
	path = append(path, current)

	if level > 10000 {
		return false, []*node{}, level
	}
	...
	//breadth part
	for i := range current.next {
		if current.next[i] == finish { //level != 0 &&
	...
	for i := range current.next {
		if _, ok := visited[current.next[i]]; !ok {
			//if !(level == 0 && current.next[i] == finish) {
			if current.next[i] != finish {
				if b, p, el := searchWPath(current.next[i], finish, visited, level+1, append(path[:0:0], path...)); b { //external links
					return b, p, el
				}
			}
		}
